        ...
        <nav class="navbar navbar-dark align-items-start sidebar sidebar-dark accordion bg-gradient-primary p-0"
            ...
            <div class="container-fluid d-flex flex-column p-0"><a
                ...
                <ul class="navbar-nav text-light" id="accordionSidebar" style="color: rgb(0,0,0);">
                    ...
                    <li class="nav-item"><a class="nav-link active" href="{{ url('/ajout-depense') }}"
        ...
        <div class="d-flex flex-column" id="content-wrapper">
            <div id="content">
                ...
                <section class="contact-clean" style="background: rgba(241,247,252,0);">
                    <form method="post" action="{{ route('enregistrer', ['modelName' => 'TypeDepense']) }}">
                        @csrf
                        ...
                        @endif
                        <h2 class="text-center">Ajout de type de dépense</h2>
                        <div class="mb-3"><label class="form-label">Nom du dépense</label><input
                                class="form-control" type="text" name="nom"
                                placeholder="Nom du nouveau acte">
                            @error('nom')
                                <span class="text-danger">{{ $message }}</span>
                            @enderror
                        </div>
                        <div class="mb-3"><label class="form-label">Budget Annuel</label><input
                                class="form-control" type="number" name="budgetannuel" placeholder="Budget annuel">
                            @error('budgetannuel')
                                <span class="text-danger">{{ $message }}</span>
                            @enderror
                        </div>
                        <div class="mb-3"><label class="form-label">Code acte</label><input class="form-control"
                    ...
                    <div class="container">
                        <div class="card shadow">
                            <div class="card-header py-3">
                                <p class="text-primary m-0 fw-bold">Depense Info</p>
                            </div>
                                    ...
                                    <table class="table my-0" id="dataTable">
                                        ...
                                        <tbody>
                                            @foreach ($typedepense as $typedepenses)
                                                <tr>
                                                    <td>{{ $typedepenses->nom }}</td>
                                                    <td>{{ number_format($typedepenses->budgetannuel, 2, ',', ' ') }}Ar
                                                    </td>
                                                    <td>{{ $typedepenses->code }}</td>
                                                    <td style="text-align: left;">
                                                        <button class="btn btn-link" type="button"
                                                            style="border: none; color: blue; text-decoration: underline;"
                                                            data-bs-toggle="modal"
                                                            data-bs-target="#modification-modal-{{ $typedepenses->id }}">
                                                            Modifier
                                                        </button>
                                                    </td>
                                                    <td style="text-align: left;">
                                                        <button class="btn btn-link" type="button"
                                                            ...
                                                            data-bs-toggle="modal"
                                                            data-bs-target="#suppression-modal-{{ $typedepenses->id }}">
                                                            Supprimer
                                                        </button>
                                                    </td>
                                                </tr>
                                                ...
                                                <div class="modal fade" role="dialog" tabindex="-1"
                                                    id="modification-modal-{{ $typedepenses->id }}">
                                                    <div class="modal-dialog" role="document">
                                                        <div class="modal-content" style="background-color: white">
                                                            ...
                                                            <div class="modal-body">
                                                                ...
                                                                <form
                                                                    action="{{ route('modifier', ['modelName' => 'TypeDepense', 'id' => $typedepenses->id]) }}"
                                                                    method="post">
                                                                    @csrf
                                                                    @if (Session::has('success'))
                                                                    ...
                                                                    <div class="mb-3"><label
                                                                            ...
                                                                            Annuel</label><input class="form-control"
                                                                            type="number" name="budgetannuel"
                                                                            value="{{ $typedepenses->budgetannuel }}">
                                                                        @error('budgetannuel')
                                                                            <span
                                                                                class="text-danger">{{ $message }}</span>
                                                                        @enderror
                                                                    </div>
                                                                    <div class="mb-3"><label class="form-label">Code
                                                                            acte</label><input class="form-control"
                                                                            type="text" name="code"
                                                                            value="{{ $typedepenses->code }}">
                                                                        @error('code')
                                                                            <span
                                                                                class="text-danger">{{ $message }}</span>
                                                                        @enderror
                                                ...
                                                <div class="modal fade" role="dialog" tabindex="-1"
                                                    id="suppression-modal-{{ $typedepenses->id }}">
                                                    <div class="modal-dialog" role="document">
                                                        <div class="modal-content" style="background-color: white">
                                                            <div class="modal-header" style="text-align: left;">
                                                                <h4 class="modal-title" style="color: black">
                                                            ...
                                                            <div class="modal-body">
                                                                <form method="POST"
                                                                    action="{{ route('supprimer', ['modelName' => 'TypeDepense', 'id' => $typedepenses->id]) }}">
                                                                    @csrf
                                                                    @if (Session::has('success'))
                                                                        <div class="alert alert-success">
                                                                            {{ Session::get('success') }}</div>
